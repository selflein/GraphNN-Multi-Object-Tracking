...
class TimeDependent(MessagePassing):
    ...
    def aggregate(self, inputs, edge_index, dim_size):
        ...
        messages_past = scatter(past_msgs, cols, dim=self.node_dim, dim_size=dim_size, reduce=self.aggr)

        # Send future messages in opposite direction of the edge
        messages_future = scatter(future_msgs, rows, dim=self.node_dim, dim_size=dim_size, reduce=self.aggr)
